...
void proccess_line(string &line, System *sys) {
  ...
  case (TIME_MSN):
    get_int(inp, mission_id), get_int(inp, st_time), get_int(inp, ed_time),
        get_int(inp, target), get_int(inp, reward);
  ...
  case (ASSIGN):
    ...
    break;
  case (RIDE):
    get_int(inp, st_time), get_int(inp, ed_time), get_int(inp, driver_id),
        get_int(inp, distance);
